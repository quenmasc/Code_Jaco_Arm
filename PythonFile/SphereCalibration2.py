...
class Sphere_Calibrator(object):
		def __init__(self,nWord,CandidateName):
				...
				self.__score=np.zeros((6),'f')
				self.__Candidate=CandidateName
				self.__best_score=np.zeros((12,6),'f')
				self.__last=sys.float_info.epsilon
				self.__best_score_init=np.zeros((12),'f')
				self.__T=np.zeros((6),'f')
				self.__P=np.zeros((6),'f')
		...
		def Sphere(self,features,Label,nClass, types):
				...
				# KMEANS below
				mu=np.zeros((self.__nClusters,3),'f')
				...
				tools.printProgressBar(0,1,prefix='Progress',suffix='Complete',decimals=1, length=50)
				#for i in range(1,nClass+1):
				km=KMeans(n_clusters=self.__nClusters,init='k-means++',n_init=10).fit(SphereData[self.__Label.T==i,:])
				mu=km.cluster_centers_
				Angle=self.SphericalAngle(km.cluster_centers_.T)
				Theta=Angle[0,:]
				Phi=Angle[1,:]
				tools.printProgressBar(1,1,prefix='Progress',suffix='Complete',decimals=1, length=50)
				params=np.zeros((2),'i')
		...
		def CalculateDiffAngle(self,AngleData, Angle):
				...
				column=len(ThetaData)
				PhiDiff=np.zeros((column,column),'f')
				ThetaDiff=np.zeros((column,column),'f')
				for i in range (column):
					for j in range (column):
								PhiDiff[i,j]=PhiData[i]-Phi[j]
								ThetaDiff[i,j]=ThetaData[i]-Theta[j]
				
				np.savetxt('phi.out',PhiDiff)
		...
		def Score(self,best_score,T,P):
				...
				last=0
				local_score=1.0
				min_score=self.__last
				for l in range(12):
						local_score*=best_score[l]
				if local_score >=self.__last or local_score >=min_score:
						self.__best_score[:,1]=best_score
						#print self.__best_score
						self.__score[1]=local_score
						#self.__best_score=np.roll(self.__best_score,1,axis=1)
						#self.__score=np.roll(self.__score,1)
						tri=np.argsort(self.__score)
						print tri
						self.__T[1]=T
						#self.__T=np.roll(self.__T,1)
						self.__P[1]=P
						#self.__P=np.roll(self.__P,1)
						self.__last=local_score
						
						score_score=self.__best_score
						self.__best_score=np.zeros((12,6),'f')
						score_local=self.__score
						score_T=self.__T
						score_P=self.__P
						self.__P=np.zeros((6),'f')
						self.__T=np.zeros((6),'f')
						...
						for i in range(len(tri)):
							self.__best_score[:,i]=score_score[:,tri[i]]
							self.__P[i]=score_P[tri[i]]
							self.__T[i]=score_T[tri[i]]
							self.__score[i]=score_local[tri[i]]
		...
		def main(self):
				...
				k=0
				tools.printProgressBar(k,T.shape[1]*T.shape[1],prefix='Progress',suffix='Complete',decimals=1, length=50)
				# check last angle point (define  z rotation throughout matrix)
				for i in range (T.shape[1]):
						for j in range (T.shape[1]):#self.__nClusters):
								...
								print best_score_init ,'\n', tools.bcolors.OKGREEN + "", self.__best_score ,'\n',self.__score,'\n' + tools.bcolors.ENDC, "Angle ", T[i,j],P[i,j], "Remaining" ,T.shape[1]**2-((i*T.shape[1])+j-1), "over", T.shape[1]**2
